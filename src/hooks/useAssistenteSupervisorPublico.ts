...
export function useAssistenteSupervisorPublico() {
  const [messages, setMessages] = useState<MensagemChat[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/assistente-supervisora`;

  const enviarMensagem = useCallback(async (input: string) => {
    ...
    try {
      ...
      console.error("Erro no assistente supervisora:", e);
      if (!error) {
  ...
  return { messages, isLoading, error, enviarMensagem, limparConversa };
}
