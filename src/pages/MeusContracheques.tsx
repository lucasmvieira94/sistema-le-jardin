...
import { useMeusContracheques, getContrachequeSignedUrl } from '@/hooks/useContracheques';
...
export default function MeusContracheques() {
  ...
  const { data, isLoading } = useMeusContracheques(funcionarioId);

  const abrir = async (path: string, download = false) => {
    const url = await getContrachequeSignedUrl(path);
    if (!url) {
      toast({ variant: 'destructive', title: 'Não foi possível abrir o holerite' });
      return;
    }
    if (download) {
      const a = document.createElement('a');
      a.href = url;
      a.download = path.split('/').pop() || 'contracheque.pdf';
      a.click();
    } else {
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="text-white">
            <h1 className="text-xl sm:text-2xl font-bold">Meus Contracheques</h1>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-green-800">
              <FileText className="w-5 h-5" /> Holerites disponíveis
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>Nenhum contracheque disponível ainda.</p>
                <p className="text-xs mt-1">Assim que o gestor publicar os holerites, eles aparecerão aqui.</p>
              </div>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        Disponibilizado em {formatarTimestampDataHora(c.created_at)}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => abrir(c.path, false)}>
                        <FileText className="w-4 h-4 mr-1" /> Ver
                      </Button>
                      <Button size="sm" onClick={() => abrir(c.path, true)} className="bg-green-700 hover:bg-green-800">
