...
import { Check, Minus, Plus, User, Droplets } from "lucide-react";
...
export const RegistroUsoFraldaForm = ({ onSuccess }: RegistroUsoFraldaFormProps) => {
  ...
  // Filtrar estoques do residente selecionado
  const estoquesResidente = estoques?.filter(
    (e) => e.residente_id === residenteId
  );
    ...
    return (
      <>
        <DialogHeader>
          ...
          <DialogDescription>
            Toque no nome do residente para continuar
          </DialogDescription>
        </DialogHeader>
  ...
  if (step === 2) {
    return (
      <>
        ...
        <ScrollArea className="h-[400px] pr-4">
          <div className="grid gap-2 py-2">
            {estoquesResidente && estoquesResidente.length > 0 ? (
              estoquesResidente.map((estoque) => (
                <Button
                  key={estoque.id}
                  variant="outline"
  ...
  // Step 3: Selecionar Funcionário
  if (step === 3) {
    return (
      <>
        <DialogHeader>
          <DialogTitle>Quem está fazendo a troca?</DialogTitle>
          <DialogDescription>
            Selecione seu nome
          </DialogDescription>
        </DialogHeader>
      ...
      <div className="space-y-6 py-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {tiposTroca.map((tipo) => (
              <Button
                ...
                className={cn(
                  "h-14 text-base",
                  tipoTroca === tipo.value && "ring-2 ring-primary"
                )}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            <span className="text-4xl font-bold min-w-[60px] text-center">
              {quantidade}
            </span>
            <Button
              type="button"
