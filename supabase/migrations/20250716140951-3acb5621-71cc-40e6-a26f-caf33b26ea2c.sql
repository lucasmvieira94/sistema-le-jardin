...
-- 4. Add missing DELETE policies for tables that don't have them
DROP POLICY IF EXISTS "Admins can delete funcionarios" ON public.funcionarios;
...
-- escalas and afastamentos already have DELETE policies, so skip them

-- 5. Ensure there's at least one admin user
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
    -- Try to find a user in auth.users
