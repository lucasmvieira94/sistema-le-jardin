-- Garantir que pgcrypto está habilitado no schema extensions
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- Recriar função com conversão correta para bytea
...
DECLARE
  code_hash TEXT;
BEGIN
  -- Gerar hash do código fornecido - converter texto para bytea
  code_hash := encode(digest(p_employer_code::bytea, 'sha256'), 'hex');
...
BEGIN
  ...
  -- Gerar hashes com conversão para bytea
  old_hash := encode(digest(p_old_code::bytea, 'sha256'), 'hex');
  new_hash := encode(digest(p_new_code::bytea, 'sha256'), 'hex');
