...
-- Atualizar a função gerar_folha_ponto_mensal para retornar horas noturnas TOTAIS
-- em vez de apenas horas EXTRAS noturnas
DROP FUNCTION IF EXISTS gerar_folha_ponto_mensal(uuid, integer, integer);
...
DECLARE
    ...
    jornada text;
    data_vigencia date;
    horas_noturnas_intervalo INTERVAL;
BEGIN
    SELECT 
    ...
    FOR dia_atual IN 1..dias_no_mes LOOP
        ...
        ELSIF registro_record.entrada IS NOT NULL AND registro_record.saida IS NOT NULL THEN
            ...
            -- Calcular TOTAL de horas noturnas (não apenas extras)
            BEGIN
