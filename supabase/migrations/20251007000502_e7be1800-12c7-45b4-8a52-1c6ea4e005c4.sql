-- Remover extensão do schema extensions e adicionar ao public
DROP EXTENSION IF EXISTS pgcrypto CASCADE;
CREATE EXTENSION IF NOT EXISTS pgcrypto SCHEMA public;

-- Recriar função com referência explícita ao schema
...
DECLARE
  code_hash TEXT;
BEGIN
  -- Gerar hash usando funções do schema public
  code_hash := public.encode(public.digest(convert_to(p_employer_code, 'UTF8'), 'sha256'), 'hex');
...
BEGIN
  ...
  -- Gerar hashes usando funções do schema public
  old_hash := public.encode(public.digest(convert_to(p_old_code, 'UTF8'), 'sha256'), 'hex');
  new_hash := public.encode(public.digest(convert_to(p_new_code, 'UTF8'), 'sha256'), 'hex');
