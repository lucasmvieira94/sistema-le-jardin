...
-- This fixes the major data exposure vulnerability

-- 1. Remove the dangerous policy that exposes all employee data publicly
DROP POLICY IF EXISTS "Allow anonymous funcionario lookup by code" ON public.funcionarios;

-- 2. Create a secure policy that only allows limited data access for valid employee codes
-- Only returns id and nome_completo for active employees during time registration
CREATE POLICY "Limited employee lookup for time registration" 
ON public.funcionarios 
FOR SELECT 
USING (
  ativo = true 
  AND EXISTS (
    SELECT 1 FROM public.registros_ponto rp 
    WHERE rp.funcionario_id = funcionarios.id 
    AND rp.created_at > now() - INTERVAL '5 minutes'
  )
);

-- 3. Remove public access to time records (major privacy violation)
DROP POLICY IF EXISTS "Allow point registration by funcionario code" ON public.registros_ponto;

-- 4. Create secure time record policies - only allow INSERT/UPDATE for active employees
CREATE POLICY "Secure time record insertion" 
...
CREATE POLICY "Secure time record updates for active employees" 
ON public.registros_ponto 
FOR UPDATE 
USING (
  funcionario_id IN (
