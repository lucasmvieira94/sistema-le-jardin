-- Fix the column definition redundancy in preencher_horarios_por_escala function
  ...
  WHILE v_data_atual <= p_data_fim LOOP
    ...
    IF v_func.escala_nome ILIKE '%12x36%' OR v_func.escala_nome ILIKE '%12%36%' THEN
      IF v_dia_ciclo % 2 = 1 THEN
        -- Chamar função sem definir colunas (função já tem OUT parameters)
        SELECT ia.intervalo_inicio, ia.intervalo_fim
        INTO v_intervals.calc_intervalo_inicio, v_intervals.calc_intervalo_fim
